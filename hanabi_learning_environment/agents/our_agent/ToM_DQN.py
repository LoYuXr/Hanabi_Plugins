...
def dqn_template(state, ToM_infer_states, num_actions, layer_size=512, num_layers=1):   ##change here!!!!!!!
  r"""Builds a DQN Network mapping states to Q-values.

  Args:
    state: A `tf.placeholder` for the RL state.
    ##### ADD #####
    ToM_infer_states: A 'tf.placeholder' for inferred RL state!!!!
    ##### ADD #####
  ...
  weights_initializer = slim.variance_scaling_initializer(
      factor=1.0 / np.sqrt(3.0), mode='FAN_IN', uniform=True)  ##初始化工具

  net = tf.cast(state, tf.float32)
  net = tf.squeeze(net, axis=2)
  #####
  net2 = tf.cast(ToM_infer_states, tf.float32)   ##新加的，需要 同样是(1, dim)维向量。需要拼接
  net = tf.concat([net, net2], axis = 0)  ##横向拼接，在信息后面加入预测信息
  ...
  net = slim.fully_connected(net, num_actions, activation_fn=None,
                             weights_initializer=weights_initializer)  ##相当于是 2*num_layers+1层网络
  return net
...
class DQNAgent(object):
  ...
  def __init__(self,
               ...
               num_players=None,
               ToM_infer_states_size = None, ##新加的，ToM推断手牌信息的结构！！！！
               gamma=0.99,
               update_horizon=1,
    ...
    with tf.device(tf_device):
      ...
      # the same weights.
      online_convnet = tf.make_template('Online', graph_template)  #tf.make_template:  给定一个任意函数,将其包装,以便它进行变量共享.
      target_convnet = tf.make_template('Target', graph_template)  #建立网络
      # The state of the agent. The last axis is the number of past observations
      # that make up the state.
      states_shape = (1, observation_size, stack_size)
      ToM_infer_shape = (1, ToM_infer_states_size)   
      ## 新加的，但是没想好如何设置格式：因为ToM出来有‘A1’这种确定性，也有‘A’：0.5这种不确定性。
      ## 同时需要仿照环境进行参数化（参见rl_env）
      
      self.state = np.zeros(states_shape)
      self.ToM_infer = np.zeros(ToM_infer_shape)   ##新加的
      
      self.state_ph = tf.placeholder(tf.uint8, states_shape, name='state_ph')  #placeholder就是接收变量
      self.ToM_infer_ph = tf.placeholder(tf.float32, ToM_infer_shape, name = 'ToM_infer_ph')  ##加的，但是没想好如何设置格式
      ...
      self._q = online_convnet(
          state=self.state_ph, ToM_infer_states= self.ToM_infer_ph, num_actions=self.num_actions)
      
      self._replay = self._build_replay_memory(use_staging)
      
      ## 记住需要改replay_memory.py!!!!!  加了self._replay.ToM_infer， self._replay.next_ToM_infer
      self._replay_qs = online_convnet(self._replay.states, self._replay.ToM_infer_states, self.num_actions)
      self._replay_next_qt = target_convnet(self._replay.next_states, self._replay.next_ToM_infer_states,
                                            self.num_actions)
  ...
  def _build_replay_memory(self, use_staging):
    ...
    return replay_memory.WrappedReplayMemory(
        ...
        ToM_infer_states_size = self.ToM_infer_states_size,   ##注意修改 WrappedReplayMemory， 加了 ToM_infer_states_size
        batch_size=32,
        stack_size=1,
  ...
  def begin_episode(self, current_player, legal_actions, observation, ToM_infer_states):  ##加了
    ...
    Args:
      ...
      ###加的
      ToM_infer_states: ToM model 手牌信息输入，可能存在维数错误！！
    ...
    ### intrinsic loss ,update params!
    
    
    self._record_transition(current_player, 0, observation, ToM_infer_states, legal_actions,  ## 加了
  ...
  def _record_transition(self, current_player, reward, observation, ToM_infer_states,
    ...
    Args:
      ...
      ## 加了
      ToM_infer_states: ToM 模型对手牌概率估计
    ...
    self.transitions[current_player].append(
        Transition(reward, np.array(observation, dtype=np.uint8, copy=True), np.array(ToM_infer_states, dtype = np.float32, copt = True),  ##加了，注意可能有误！！未确定ToM_infer_states 格式
  ...
  def _post_transitions(self, terminal_rewards):
    ...
    for player in range(self.num_players):
      ...
      for index, transition in enumerate(self.transitions[player]):  ##循环加入
        ...
        self._store_transition(transition.observation, transition.ToM_infer_states, transition.action, ##加了
  ...
  def _select_action(self, observation, ToM_infer_states ,legal_actions):
    ...
    Args:
      ...
      ##新加的
      ToM_infer_states: ToM模型推断手牌信息, 可能存在bug!!
  ...
  def _train_step(self):   ####大概率会有错误，检查未改动
    ...
    if (self._replay.memory.add_count > self.min_replay_history and
      ...
      ##改了， prefetch_batch return 一个 
      # (states, ToM_infer_states, actions, rewards, next_states, next_ToM_infer_states, terminals, indices, next_legal_actions)
  ...
  def _store_transition(self, observation, ToM_infer_states, action, reward, is_terminal,  ##加了，注意replay_memory修改
                        legal_actions):
    ...
    if not self.eval_mode:
      self._sess.run(
          self._replay.add_transition_op, {
              ...
              self._replay.add_ToM_ph: ToM_infer_states,  ##加了
  ...
  def bundle_and_checkpoint(self, checkpoint_dir, iteration_number):
    ...
    bundle_dictionary['ToM_infer'] = self.ToM_infer
